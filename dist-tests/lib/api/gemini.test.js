...
describe('analyzeWithGemini', () => {
    it('omits verbose fields from the prompt payload', async () => {
        ...
        try {
            ...
            const payload = extractPromptPayload(capturedPrompt);
    ...
    it('truncates lengthy answers to honour the prompt length limit', async () => {
        ...
        try {
            ...
            assert.ok(capturedPrompt.length <= Number(process.env.VITE_GEMINI_MAX_PROMPT_CHARS), 'prompt should not exceed configured limit');
            assert.ok(warnings.some(([message]) => typeof message === 'string' && message.includes('Prompt truncated')), 'should log a truncation warning');
        }
        finally {
    ...
    it('throws a descriptive error when Gemini blocks the response', async () => {
        ...
        const originalFetch = globalThis.fetch;
        globalThis.fetch = async () => new Response(JSON.stringify(blockedResponse), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });
        try {
            const { analyzeWithGemini, GeminiApiError } = await loadGeminiModule();
            await assert.rejects(() => analyzeWithGemini(request), (error) => {
                assert.ok(error instanceof GeminiApiError);
                assert.match(error.message, /did not include any content parts/, 'error message should describe the missing content parts');
    ...
    it('surfaces a helpful error when Gemini is unavailable', async () => {
        ...
        const originalFetch = globalThis.fetch;
        globalThis.fetch = async () => new Response(JSON.stringify(unavailablePayload), {
            status: 503,
            headers: { 'Content-Type': 'application/json' },
        });
        try {
